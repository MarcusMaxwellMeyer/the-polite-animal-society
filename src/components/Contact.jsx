  ...
  const infoCard = (Icon, title, text) => (
    <div className="bg-white/70 backdrop-blur-sm border border-warm/25 rounded-2xl shadow-sm p-6 flex items-center gap-4">
      ...
      <div>
        ...
        <p className="font-body text-sm text-bark/50">{text}</p>
      </div>
    </div>
  );

  return (
    <section id="contact" ref={sec} className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-16">
          ...
          <h2 className="font-serif text-4xl md:text-5xl font-bold text-bark mb-5">Write to Us</h2>
        ...
        <div className="grid lg:grid-cols-5 gap-12 lg:gap-16 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.25 }} className="lg:col-span-3">
            <div className="bg-white/70 backdrop-blur-sm border border-warm/25 rounded-2xl shadow-sm p-8 md:p-10">
              {sent ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16">
                  <span className="text-5xl block mb-4">💌</span>
                  <h3 className="font-serif text-2xl font-bold text-bark mb-2">Thank You!</h3>
                  <p className="font-body text-bark/50">Your message has been sent. Our owl secretary will respond shortly.</p>
                </motion.div>
              ) : (
                <form onSubmit={submit} className="flex flex-col gap-5">
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="font-body text-sm text-bark/60 mb-1.5 block">Your Name</label>
                      <input type="text" required placeholder="e.g. Beatrix Potter"
                        className="w-full bg-cream/50 border border-warm/30 rounded-xl px-4 py-3 font-body text-sm text-bark placeholder:text-bark/25 focus:outline-none focus:border-forest/50 focus:ring-2 focus:ring-forest/10 transition-all" />
                    </div>
                    <div>
                      <label className="font-body text-sm text-bark/60 mb-1.5 block">Email</label>
                      <input type="email" required placeholder="beatrix@example.com"
                        className="w-full bg-cream/50 border border-warm/30 rounded-xl px-4 py-3 font-body text-sm text-bark placeholder:text-bark/25 focus:outline-none focus:border-forest/50 focus:ring-2 focus:ring-forest/10 transition-all" />
                    </div>
                  </div>
                  <div>
                    <label className="font-body text-sm text-bark/60 mb-1.5 block">Subject</label>
                  ...
                  <div>
                    <label className="font-body text-sm text-bark/60 mb-1.5 block">Message</label>
                    <textarea required rows={5} placeholder="Tell us about what you're looking for..."
                      className="w-full bg-cream/50 border border-warm/30 rounded-xl px-4 py-3 font-body text-sm text-bark placeholder:text-bark/25 focus:outline-none focus:border-forest/50 focus:ring-2 focus:ring-forest/10 transition-all resize-none" />
                  </div>
