...
export default function About() {
  ...
  return (
    <section id="about" ref={sec} className="py-24 md:py-32 bg-linen/40 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className="relative">
            <div className="bg-gradient-to-br from-warm/25 to-sage/15 rounded-3xl aspect-square flex items-center justify-center relative">
              <div className="text-center">
                <span className="text-[7rem] block mb-3">🎨</span>
                <p className="font-hand text-2xl text-bark/50">Our Studio</p>
              </div>
            ...
            <div className="absolute -bottom-5 -right-5 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-body text-xs text-bark/40">Countryside Studio</p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.15 }}>
            ...
            <h2 className="font-serif text-4xl md:text-5xl font-bold text-bark mb-8 leading-tight">
              Where Whimsy Meets <span className="text-forest">Watercolour</span>
            </h2>
            <div className="space-y-5 font-body text-bark/60 text-base md:text-lg leading-relaxed">
            ...
            <div className="grid sm:grid-cols-3 gap-8 mt-12">
              {values.map((v, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4 + i * 0.12 }}>
                  ...
                  <p className="font-body text-xs text-bark/45 leading-relaxed">{v.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
